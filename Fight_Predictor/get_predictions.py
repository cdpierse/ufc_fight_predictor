...
class GetPredictions:
    ...
    def predict_bout_winner(self):
        ...
        return model.predict_classes(scaled), implied_abs_probability[0]
...
if __name__ == ('__main__'):
    ...
    fight_card = [{'fighter1':'Daniel Cormier',
                 'fighter2':'Stipe Miocic','Weight Class': 11},
                 {'fighter1':'Nate Diaz',
                 'fighter2':'Anthony Pettis','Weight Class': 8},
                 {'fighter1':'Yoel Romero',
                 'fighter2':'Paulo Costa','Weight Class': 9},
                 {'fighter1':'Gabriel Benitez',
                 'fighter2':'Sodiq Yusuff','Weight Class': 6},
                 {'fighter1':'Derek Brunson',
                 'fighter2':'Ian Heinisch','Weight Class': 9},
                 {'fighter1':'Raphael Assuncao',
                 'fighter2':'Cory Sandhagen','Weight Class': 5},
                 {'fighter1':'Christos Giagos',
                 'fighter2':'Drakkar Klose','Weight Class': 7},
                 {'fighter1':'Casey Kenney',
                 'fighter2':'Manny Bermudez','Weight Class': 5},
                 {'fighter1':'Jodie Esquibel',
                 'fighter2':'Hannah Cifers','Weight Class': 0},
                 {'fighter1':'Kyung Ho Kang',
                 'fighter2':'Brandon Davis','Weight Class': 5},
                 {'fighter1':'Shana Dobson',
                 'fighter2':'Sabina Mazo','Weight Class': 1}]
    for bout in fight_card:
        gp = GetPredictions()
        fighter1_name =  bout['fighter1']
        fighter2_name = bout['fighter2']
        fighter1 = gp.get_fighter_details(fighter1_name)
        fighter2 = gp.get_fighter_details(fighter2_name)
    
        gp.create_fighter_dataframe(fighter1, fighter2, bout['Weight Class'])
        gp.create_final_df()
        
        try:
            result, probability = gp.predict_bout_winner()
            if result[0] == 0:
                print(f'Predicted Winner of {fighter1_name} vs {fighter2_name} is {fighter1_name} with implied probability {probability}')
            else:
                print(f'Predicted Winner of {fighter1_name} vs {fighter2_name} is {fighter2_name} with implied probability {probability}')
        except Exception:
            print(f'Could not predict winners for {fighter1_name} and {fighter2_name}')
            pass
